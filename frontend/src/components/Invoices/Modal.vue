      ...
      <form>
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="text" placeholder="19 Union Terrace" autocomplete="off" name="company_address"
              v-model="newInvoice.senderAddress.street" />
          </div>
          <div class="input-wrapper-grid">
            <div class="input-wrapper">
              <label for="city">Ville</label>
              <input type="text" v-model="newInvoice.senderAddress.city" placeholder="Paris" name="city"
                autocomplete="off" />
            </div>
            <div class="input-wrapper">
              <label for="company_postal_code">Code postal</label>
              <input v-model="newInvoice.senderAddress.postCode" type="text" placeholder="75000"
                name="company_postal_code" autocomplete="off" />
            </div>
            <div class="input-wrapper">
              <label for="company_country">Pays</label>
              <input type="text" v-model="newInvoice.senderAddress.country" autocomplete="off" placeholder="France"
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="text" v-model="newInvoice.clientName" placeholder="John Doe" autocomplete="off"
              name="clientName" />
          </div>
          <div class="input-wrapper">
            <label for="email">Email</label>
            <input v-model="newInvoice.clientEmail" type="text" placeholder="john@example.com" name="email"
              autocomplete="off" />
          </div>
          <div class="input-wrapper">
            <label for="client_address">Adresse</label>
            <input type="text" placeholder="" name="client_address" autocomplete="off"
              v-model="newInvoice.clientAddress.street" />
          </div>
          <div class="input-wrapper-grid" style="--columns: 3">
            <div class="input-wrapper">
              <label for="country">Ville</label>
              <input type="text" autocomplete="off" placeholder="Paris" name="country"
                v-model="newInvoice.clientAddress.city" />
            </div>
            <div class="input-wrapper">
              <label for="country">Code postal</label>
              <input type="text" autocomplete="off" placeholder="75000" name="country"
                v-model="newInvoice.clientAddress.postCode" />
            </div>
            <div class="input-wrapper">
              <label for="country">Pays</label>
              <input type="text" v-model="newInvoice.clientAddress.country" placeholder="France" name="country"
                autocomplete="off" />
            </div>
          </div>
          ...
          <div class="input-wrapper-grid" style="--columns: 2">
            <div class="input-wrapper">
              ...
              <input type="date" autocomplete="off" placeholder="02/02/2023" name="createdAt"
                v-model="newInvoice.createdAt" />
            </div>
            <div class="input-wrapper">
              <label for="country">Condition de paiement</label>
              <select-input :options="paymentTerms" @input="onSelectInput($event)"></select-input>
            </div>
          </div>
        </div>
        ...
        <div class="table">
          ...
          <row-product v-for="(item, i) in newInvoice.items" :key="i" :item="item"
            @delete="deleteRowProduct(i)"></row-product>
        </div>
        <div v-if="newInvoice.items.length <= 0" class="empty-row">
          <p>Cliquer sur le bouton &#171;
          <pre>+Ajouter un produit</pre> &#187; pour ajouter une ligne.</p>
        </div>
        <button type="button" class="button__invoice-add" @click.prevent="addNewRowProduct">
          + Ajouter un produit
        </button>
        <div class="modal__footer">
          <button type="button" class="button__cancel" @click="$emit('close')">Annuler</button>
          <div class="button__wrapper">
            <button type="button" @click.prevent="$emit('invoice-events', newInvoice, 'draft')"
              class="button__save-draft">
              Sauvegarder le brouillon
            </button>
            <button type="button" @click.prevent="$emit('invoice-events', newInvoice, 'pending')" class="button__create">
              Créer la facture
            </button>
          </div>
        </div>
